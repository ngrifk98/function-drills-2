...
const findGrapeExpression = function (arr) {
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].color === "purple") {
            return console.log(`The fruit with index ${arr.indexOf(arr[i])} is a grape`);
        }
...
function pricesPlusTax(prices, callback) {
  for (let i = 0; i < prices.length; i++) {
    const tax = prices[i] * 0.2;
    const totalPrice = prices[i] + tax;
...
const newTimesFour = multiplyingFactory(5);
newTimesFour(4);
